...
export default function ProfileForm() {
  ...
  useEffect(() => {
    ...
    if (user.role !== "worker") {
      router.push("/"); // prevent non-workers from accessing
      return;
    }
    ...
    const fetchProfile = async () => {
      try {
        ...
        if (data.length > 0) {
          const workerProfile = data[0];

          // ✅ if worker already has profile (profession + phone filled), redirect
          if (workerProfile.profession && workerProfile.phone) {
            router.push("/worker/dashboard");
            return;
          }

          // else load form with existing (partially filled) data
          setFormData(workerProfile);
          setPicPreview(workerProfile.profilePic || null);
        }
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (existing.length > 0) {
        await axios.put(
          `http://localhost:50001/workers/${existing[0].id}`,
          workerData
        );
      } else {
